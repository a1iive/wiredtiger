...
int CreateHistogramBucketMapper(HistogramBucketMapper* mapper) {
    ...
    // printf("size:%d %f\n",temp->size_, my_sqrt(4.2));
    // for(int i =0;i<temp->size_;i++){
    //     printf("%llu ",temp->bucketValues_[i]);
    // }
    // printf("\n");
    return 0;
}
...
void HistogramStatClear(HistogramStat *stat) {
    ...
    atomic_init(&stat->max_, (uint64_t)~0);
...
void HistogramStatAdd(HistogramStat *stat, uint64_t value) {
    ...
    const size_t index = IndexForValue(&stat->bucketMapper_, value);
    assert(index < stat->num_buckets_);
    (void)__wt_atomic_add64(&(stat->buckets_[index]), 1);

    uint64_t old_min = stat->min_;
    if (value < old_min) {
        __wt_atomic_cas64(&stat->min_, value, old_min);
    }

    uint64_t old_max = stat->max_;
    if (value > old_max) {
        __wt_atomic_cas64(&stat->max_, value, old_max);
    }

    (void)__wt_atomic_add64(&(stat->num_), 1);
    (void)__wt_atomic_add64(&(stat->sum_), value);
    (void)__wt_atomic_add64(&(stat->sum_squares_), value * value);
}

void HistogramStatMerge(HistogramStat *stat, const HistogramStat *other) {
    ...
    uint64_t old_min = stat->min_;
    uint64_t other_min = other->min_;
    while (other_min < old_min &&
            __wt_atomic_cas64(&stat->min_, old_min, other_min)) {}

    uint64_t old_max = stat->max_;
    uint64_t other_max = other->max_;
    while (other_max > old_max &&
            __wt_atomic_cas64(&stat->max_, old_max, other_max)) {}

    uint64_t other_num = other->num_;
    (void)__wt_atomic_add64(&(stat->num_), other_num);

    uint64_t other_sum = other->sum_;
    (void)__wt_atomic_add64(&(stat->sum_), other_sum);
    
    uint64_t other_sum_squares = other->sum_squares_;
    (void)__wt_atomic_add64(&(stat->sum_squares_), other_sum_squares);

    for (unsigned int b = 0; b < stat->num_buckets_; b++) {
        uint64_t other_bucket = other->buckets_[b];
        (void)__wt_atomic_add64(&(stat->buckets_[b]), other_bucket);
    }
}

double HistogramStatPercentile(HistogramStat *stat, double p) {
    uint64_t num = stat->num_;
    double threshold = num * (p / 100.0);
    uint64_t cumulative_sum = 0;
    for (unsigned int b = 0; b < stat->num_buckets_; b++) {
        uint64_t bucket_value = stat->buckets_[b];
        ...
        if (cumulative_sum >= threshold) {
            ...
            uint64_t cur_min = stat->min_;
            uint64_t cur_max = stat->max_;
            if (r < cur_min) r = (double)cur_min;
            if (r > cur_max) r = (double)cur_max;
            return r;
        }
    }
    double max = (double)stat->max_;
    return max;
}
...
double HistogramStatAverage(HistogramStat *stat) {
    uint64_t cur_num = stat->num_;
    uint64_t cur_sum = stat->sum_;
    if (cur_num == 0) return 0;
    return (double)(cur_sum) / (double)(cur_num);
}

// 和
uint64_t HistogramStatSum(HistogramStat *stat) {
    return stat->sum_;
}
...
double HistogramStatStandardDeviation(HistogramStat *stat) {
    uint64_t cur_num = stat->num_;
    uint64_t cur_sum = stat->sum_;
    uint64_t cur_sum_squares = stat->sum_squares_;
    if (cur_num == 0) return 0;
    double variance =
...
char* HistogramStatToString(HistogramStat *stat) {
    ...
    uint64_t min = stat->min_;
    uint64_t max = stat->max_;
    snprintf(buf, sizeof(buf),
...
uint64_t HistogramImplSum(OBJ impl) {
    if(impl == NULL) return 0;
    HistogramImpl *histogram_impl = (HistogramImpl*)impl;
    return HistogramStatSum(&histogram_impl->stats_);
}

uint64_t HistogramImplMin(OBJ impl) {
    if(impl == NULL) return 0;
    HistogramImpl *histogram_impl = (HistogramImpl*)impl;
    return histogram_impl->stats_.min_;
}

uint64_t HistogramImplMax(OBJ impl) {
    if(impl == NULL) return 0;
    HistogramImpl *histogram_impl = (HistogramImpl*)impl;
    return histogram_impl->stats_.max_;
}

int CreateHistogramImpl(HistogramImpl *histogram_impl) {
    HistogramImpl *obj = histogram_impl;
    if(obj->init_) return 0;
    int ret = CreateHistogramStat((HistogramStat**)&obj->stats_);
    if (ret != 0) return -1;
    // LockInit(&obj->lock_);
    ...
    obj->interface.Sum = HistogramImplSum;
    obj->interface.Min = HistogramImplMin;
    obj->interface.Max = HistogramImplMax;
